...
class VerificationCodeRateLimiterTest {
  ...
  void first_call_allowed() {
    assertThatCode(() -> limiter.check("a@example.com", "1.1.1.1")).doesNotThrowAnyException();
  }

  @Test
  @DisplayName("동일_이메일_60초_내_재요청_차단_retryAfter반환")
  void cooldown_60s_blocks_second() {
    limiter.check("a@example.com", "1.1.1.1");
    clock.advance(Duration.ofSeconds(30));

    assertThatThrownBy(() -> limiter.check("a@example.com", "2.2.2.2"))
  ...
  void email_5_per_hour_then_blocked() {
    for (int i = 0; i < 5; i++) {
      limiter.check("e@example.com", "1.1.1.1");
      clock.advance(Duration.ofMinutes(2));
    }
    assertThatThrownBy(() -> limiter.check("e@example.com", "1.1.1.1"))
        .isInstanceOf(RateLimitException.class);
  }
  ...
  void ip_10_per_hour_then_blocked() {
    for (int i = 0; i < 10; i++) {
      limiter.check("user" + i + "@example.com", "9.9.9.9");
      clock.advance(Duration.ofMinutes(1));
    }
    assertThatThrownBy(() -> limiter.check("user99@example.com", "9.9.9.9"))
        .isInstanceOf(RateLimitException.class);
  }
  ...
  void window_resets_after_one_hour() {
    ...
    assertThatCode(() -> limiter.check("e@example.com", "1.1.1.1")).doesNotThrowAnyException();
  }
  ...
  void null_ip_skips_ip_check() {
    for (int i = 0; i < 4; i++) {
      limiter.check("u" + i + "@example.com", null);
      clock.advance(Duration.ofMinutes(1));
    }
    assertThatCode(() -> limiter.check("u99@example.com", null)).doesNotThrowAnyException();
  }
